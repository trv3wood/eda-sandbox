"""Convert the deterministic UHDM structure export into RTL graph facts."""
...
def _unqualified(value: object) -> str:
    """Return the SystemVerilog name without a UHDM library qualification."""
...
def _records(value: object, field: str) -> list[dict[str, Any]]:
    ...
    if not isinstance(value, list) or not all(isinstance(item, dict) for item in value):
        raise ValueError(f"UHDM structure field {field} must be a list of objects")
    return value
...
def structure_to_graph(
    ...
    """Create RTL entities and relationships from an official UHDM export.
    ...
    if not isinstance(structure, dict):
        raise ValueError("UHDM structure must be a JSON object")
    ...
    if not modules or not top_modules:
        raise ValueError("UHDM structure must contain definition and top modules")
    ...
    def add_definition(record: dict[str, Any]) -> dict[str, Any]:
        ...
        if not name:
            raise ValueError("UHDM module definition is missing a name")
        ...
        for field, entity_type, properties in (
            ...
            for child in _records(record.get(field), field):
                ...
                if not isinstance(child_name, str) or not child_name:
                    raise ValueError(f"UHDM {field} entry is missing a name")
    ...
    def add_instance(record: dict[str, Any], parent: dict[str, Any] | None, is_top: bool) -> None:
        ...
        if not name:
            raise ValueError("UHDM elaborated module is missing a name")
        refs = _source_ref(record, project, sources)
