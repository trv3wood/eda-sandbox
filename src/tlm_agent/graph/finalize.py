...
def align_cross_source(
    ...
    """以保守的精确匹配连接可选 Spec 实体和 RTL 实体。

    这里只接受唯一的实体名或完整 token 命中，避免把同名端口、缩写等
    不确定关联写成 canonical fact。确定性文本单元仍可直接作为合同证据，
    不依赖这里生成跨源边。
    """
    # 只有名字在 RTL 图中唯一时，才允许把它作为跨源匹配候选。
    ...
    for spec in spec_entities:
        ...
        for alias, rtl in unique.items():
            ...
            if any(re.search(pattern, text) for text in texts):
                # Spec 实体的原文提到了唯一 RTL 名称，记录可追溯的提及边。
                result.append(
                    relationship(
...
def finalize_graph(
    ...
    *,
    ...
    """发布唯一可供合同、审批和生成阶段消费的 canonical graph。

    ``structure`` 是通过所选 EDA 后端校验后的结构快照；本函数将其扁平化
    为 JSONL RTL 图，再与可选的 Spec 图合并、校验并记录摘要。查询层
    ``graph.runtime`` 不参与此过程。
    """
    graph = project / ".systemc-agent" / "graph"
    manifest = load_json(graph / "manifest.json")
    # Spec producer 可以显式跳过，但不能处于未完成状态；否则不能发布混合图。
    ...
    if structure is not None:
        # 将嵌套的 EDA 导出转换成逐行实体和关系，便于稳定引用和流式处理。
        ...
        manifest["producers"]["rtl"] = {
            ...
            "backend": (
                "uhdm"
                if structure.get("backend") == "uhdm-python-vpi"
                else structure.get("backend")
            ),
    ...
    # 从本次已发布的 producer 产物读取，而不是再次分析 RTL 或文档。
    ...
    # 这两份合并文件是后续 architecture/approval/generate 的唯一图入口。
    ...
    # 对 RTL 输入额外检查：elaboration 结果只能有一个被标记的顶层实例。
    top_instances = [
        item
    ...
    # manifest 固定所有 canonical 输入的摘要；tools/ 下的可再生产物不在其中。
    artifact_names = (
        "document_tree.json",
        "text_units.jsonl",
        "spec_entities.jsonl",
    ...
    # 一个稳定的内容摘要用于检测图在审批前后是否发生了语义载荷变化。
